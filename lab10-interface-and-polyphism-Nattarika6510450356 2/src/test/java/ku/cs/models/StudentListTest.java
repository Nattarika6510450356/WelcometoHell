...
class StudentListTest {
    ...
    @Test
    @DisplayName("find student by Id & Object are same or diffrent?")
    void testFindStudentByObject() {
        StudentList s = new StudentList();
        
        Student lisa = new Student("1", "Lisa");

        s.addNewStudent("1", "Lisa");

        Student foundStudent = s.findStudentByObject(lisa);

        assertNotNull(foundStudent);
        assertEquals(lisa, foundStudent);
    }
